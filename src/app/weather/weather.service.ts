...
export class WeatherService {
  ...
  getCityNameByCoordinates(latitude: number, longitude: number){
    const URL = CITY_API_URL.replace('{{API_KEY}}', environment.cityApiKey).replace('{{Latitude}}', latitude.toString()).replace('{{Longitude}}', longitude.toString());
    return this.http.get(URL).pipe(map((response: GeolocationResponse) => response));
  }
}
